...
char* LCipher::b64_e(const char* message, int& iSize){
	int len = Misc::StrLen((c_char*)message);
	iSize = b64_encoded_size(len);
...
u64 LCipher::GetCipheredFileSize(const std::string strFilename){
	u64 uTmp = Misc::GetFileSize(strFilename);
	return b64_encoded_size(uTmp);	
}

u64 LCipher::GetCipheredFileSize2(const std::string strFilename, unsigned int iBlockSize){
	u64 uFileSize = Misc::GetFileSize(strFilename);
	u64 uRet = 0;
	while(1){
		if(uFileSize < iBlockSize){
			uRet += b64_encoded_size(uFileSize);
			break;
		} else {
			uFileSize -= iBlockSize;
			uRet += b64_encoded_size(iBlockSize);
		}
	}
	return uRet;
}
